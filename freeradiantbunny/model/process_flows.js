...
function ProcessFlows() {
	...
	if (idOrNoId) {
		...
		sql = "select z.status, z.sort, z.id, z.name, p.name, c.name from process_flows z, processes p, processes c where z.id = " + idOrNoId + " and z.parent_processes_id = z.parent_processes_id AND z.child_processes_id = c.id AND z.publish = 'true';";
	    } else {
		sql = sqlgenerator.getStandardSingle(this.name, this.schema, idOrNoId, this.inboundForeignKeyTables, paramUpkIsValid);
		// refactor
		//sql = "select z.status, z.sort, z.id, z.name, p.name as parent_process, c.name as child_process from process_flows z, processes p, processes c where z.id = " + idOrNoId + " and z.parent_processes_id = p.id AND z.child_processes_id = c.id AND z.publish = 'true';";
	    }
	} else {
	    ...
	    sql = "select z.status, z.sort, z.id, z.name, z.parent_processes_id, z.child_processes_id from process_flows z where z.publish ='true' " + orderBy + ";";
	}
	return sql;
    };
}
