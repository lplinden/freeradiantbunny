...
function SeedPackets() {
	...
	if (typeof idOrNoId !== 'undefined' && idOrNoId !== "") {
	    ...
	    // array(select count(ph.id) from plant_histories ph, seed_packets sp where sp.id = ph.seed_packet_id) as plant_histories_count 
            ...
            sql = "select sp.id, sp.variety_id, sp.supplier_id, sp.url, sp.contents, sp.sow_instructions, sp.days_to_maturity, array(select count(ph.id) from plant_histories ph where sp.id = ph.seed_packet_id) as plant_histories_count from seed_packets sp " + orderBy;
        }
        return sql;
    };
}
