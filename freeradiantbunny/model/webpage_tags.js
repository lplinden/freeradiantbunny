...
function WebpageTags() {
    ...
    this.getSql = function (idOrNoId, classNameFilter, paramSort, specialFlag, queryTerms) {
        ...
        var orderBy;
        if (idOrNoId) {
            sql = "select z.id, z.webpage_id, z.tag_id from webpage_webpage_tags z WHERE z.id = " + idOrNoId + ";";
        } else {
            orderBy = "ORDER BY t.name, w.name, w.id";
            debug("webpage_tags orderBy =", orderBy);
            sql = "select wt.id, wt.webpage_id, w.name as webpage_name, wt.tag_id, t.name as tag_name from webpage_webpage_tags wt, webpages w, webpage_tags t where w.id = wt.webpage_id AND t.id = wt.tag_id " + orderBy + ";";
        }
        return sql;
    };
}
