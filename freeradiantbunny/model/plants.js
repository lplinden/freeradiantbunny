...
function Plants() {
	...
	if (typeof idOrNoId !== 'undefined' && idOrNoId !== "") {
	    ...
	    // old version
            //sql = "SELECT u.id, u.name, u.botanical_name, pf.name as plant_family, pc.name as plant_category FROM plants u, plant_categories pc, plant_families pf where u.plant_category_id = pc.id AND u.plant_family_id = pf.id " + orderBy;
	    // new version that gets all plants (even if not category)
            sql = "SELECT u.id, u.sort, u.name, u.botanical_name, pf.name as plant_family FROM plants u, plant_families pf WHERE u.plant_family_id = pf.id " + orderBy;
        }
        return sql;
    };
}
