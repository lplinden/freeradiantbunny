...
function Domains() {
	...
	debug("domains classNameFilter =", classNameFilter);
	    ...
	    sql = "select z.status, z.sort, z.id, concat('<a href=\"/domains/', z.id, '\">', z.tli , '</a>') as tli, z.img_url as img, concat('<a href=\"http', z.ssl_cert, '://', z.domain_name, '\">', z.domain_name, '</a>') as domain_name, z.name, z.description as description, concat('<a href=\"/webpages/domains/', z.id,";
	    if (paramUpkIsValid) {
		sql += " '?" + paramUpkIsValid + "',";
	    }
